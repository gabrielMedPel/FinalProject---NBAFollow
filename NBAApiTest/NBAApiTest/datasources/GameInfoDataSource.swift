...
struct GameInfoDataSource {
    ...
    func getGameInfo(date: String, gameId: String, callback: @escaping (NBAGameInfoResponse?, NBAError?)->Void) {
        
        let urlString = "\(baseUrl)/data/10s/prod/v1/\(date)/\(gameId)_boxscore.json"
        ...
        URLSession.shared.dataTask(with: request) { (data, response, err) in
            ...
            do{
                ...
                let nbaInfo = try decoder.decode(NBAGameInfoResponse.self, from: data)
